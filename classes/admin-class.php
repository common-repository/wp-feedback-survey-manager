...
<div class="metabox-holder">
    ...
    public function on_load_page() {
        ...
        add_meta_box('wp_feedback_pinfo_op', __('FeedBack Manager &raquo; Personal Information Options', 'fbsr'), array(&$this, 'meta_pinfo'), $this->pagehook, 'normal');
	...
	'<p>' . sprintf(__('<a href="%s" target="_blank">Support Forums</a>', 'fbsr'), wp_feedback_loader::$support_forum) . '</p>' .
        '<p>' . '<a href="http://codecanyon.net/item/wp-feedback-survey-quiz-manager-pro/3180835?ref=iPanelThemes" class="button-primary">' . __('Go Pro', 'fbsr') . '</a></p>'
	);
    }
    ...
    public function save_post() {
        ...
        foreach(array('f_name', 'l_name', 'email', 'phone') as $pkey) {
            $wp_feedback_pinfo[$pkey] = array(
                'enabled' => isset($this->post['pinfo'][$pkey]['enabled']) ? true : false,
                'required' => isset($this->post['pinfo'][$pkey]['required']) ? true : false,
            );
        }
        ...
        for($i = 0; $i < 20; $i++) {
            $wp_feedback_survey[$i] = array(
                ...
                'required' => isset($this->post['survey'][$i]['required']) ? true : false,
            );
            $wp_feedback_feedback[$i] = array(
                ...
                'required' => isset($this->post['feedback'][$i]['required']) ? true : false,
            );
            $wp_feedback_pinfo[$i] = array(
                'enabled' => isset($this->post['pinfo'][$i]['enabled']) ? true : false,
                'question' => $this->post['pinfo'][$i]['question'],
                'options' => trim(preg_replace("/\r\n\s*[\r\n]*/", "\r\n", $this->post['pinfo'][$i]['options']), "\r\n"),
                'type' => $this->post['pinfo'][$i]['type'],
                'required' => isset($this->post['pinfo'][$i]['required']) ? true : false,
            );
        }

        $updates = array(
            'wp_feedback_global', 'wp_feedback_survey', 'wp_feedback_feedback', 'wp_feedback_pinfo'
        );
...
<table class="form-table">
    <tbody>
        ...
        <tr>
            <th scope="col">
                <?php _e('Tab Order', 'fbsr'); ?>
            </th>
            <td>
                <ul id="wp_feedback_sortable" class="sortable_ul" data-value="h_global_tab_order">
                    <?php foreach($op['tab_order'] as $pos => $tab) : ?>
                    <li id="global_tab_order_<?php echo $pos; ?>" data-value="<?php echo $tab; ?>"><?php echo $titles[$tab]; ?></li>
                    <?php endforeach; ?>
                </ul>
                <?php foreach($op['tab_order'] as $pos => $tab) : ?>
                <input type="hidden" name="global[tab_order][<?php echo $pos; ?>]" id="h_global_tab_order_<?php echo $pos; ?>" value="<?php echo esc_attr($tab); ?>" />
                <?php endforeach; ?>
            </td>
            <td>
                <span class="help">
                    <?php _e('Drag and position the tabs', 'fbsr'); ?>
                </span>
            </td>
        </tr>
        <tr>
...
<div class="survey-<?php echo $i; ?>">
    <table class="form-table">
        <tbody>
            ...
            <tr>
                <th scope="row">
                    <label for="survey_<?php echo $i; ?>_required"><?php _e('Compulsory?', 'fbsr'); ?></label>
                </th>
                <td>
                    <?php $this->print_checkbox('survey[' . $i . '][required]', 'true', (true == $op[$i]['required'])); ?>
                </td>
                <td>
                    <span class="help">
                        <?php _e('If you want to make  this question compulsory, then tick, otherwise untick.'); ?>
                    </span>
                </td>
            </tr>
        </tbody>
...
<div class="feedback-<?php echo $i; ?>">
    <table class="form-table">
        <tbody>
            ...
            <tr>
                <th scope="row">
                    <label for="feedback_<?php echo $i; ?>_required"><?php _e('Compulsory?', 'fbsr'); ?></label>
                </th>
                <td>
                    <?php $this->print_checkbox('feedback[' . $i . '][required]', 'true', (true == $op[$i]['required'])); ?>
                </td>
                <td>
                    <span class="help">
                        <?php _e('If you want to make  this question compulsory, then tick, otherwise untick.'); ?>
...
<?php endfor; ?>
    ...
    public function meta_pinfo() {
        $op = get_option('wp_feedback_pinfo');
        $predefined = array();

        $pinfos = array(
            'f_name' => __('First Name', 'fbsr'),
            'l_name' => __('Last Name', 'fbsr'),
            'email' => __('Email Address', 'fbsr'),
            'phone' => __('Phone Number', 'fbsr'),
        );

        $pinfo_type = array(
            'single' => __('Single Value from the Options', 'fbsr'),
            'multiple' => __('Multiple Value from the Options', 'fbsr'),
            'free-input' => __('Free Type Text using a small Input Box', 'fbsr'),
            'free-text' => __('Free Type Text using a Textarea', 'fbsr'),
            'required-checkbox' => __('A Required Checkbox', 'fbsr'),
        );
        ?>
<ul class="metabox-tabs">
    <li class="tab pinfo-predefined"><a href="javascript:void(null);" title="<?php _e('Predefined Personal Informations', 'fbsr'); ?>" class="active"><?php _e('Predefined', 'fbsr'); ?></a></li>
    <?php for($i = 0; $i <= 19; $i++) : ?>
    <li class="tab pinfo-<?php echo $i; ?>"><a href="javascript:void(null);" title="<?php echo __('Personal Information Question', 'fbsr') . ' ' . ($i+1); ?>" class=""><?php echo ($i+1); ?></a></li>
    <?php endfor; ?>
</ul>
<div class="pinfo-predefined">
    <table class="form-table">
        <?php foreach($pinfos as $pinfo => $name) : ?>
        <?php $predefined[$pinfo] = $op[$pinfo]; ?>
        <tr>
            <td>
                <label for="pinfo_<?php echo $pinfo ?>"><?php echo $name; ?></label>
            </td>
            <td>
                <label for="pinfo_<?php echo $pinfo ?>_enabled">
                    <?php $this->print_checkbox('pinfo[' . $pinfo . '][enabled]', 'true', (true == $predefined[$pinfo]['enabled'])); ?> <?php _e('Enabled?', 'fbsr'); ?>
                </label>
                <label for="pinfo_<?php echo $pinfo ?>_required">
                    <?php $this->print_checkbox('pinfo[' . $pinfo . '][required]', 'true', (true == $predefined[$pinfo]['required'])); ?> <?php _e('Required?', 'fbsr'); ?>
                </label>
            </td>
            <td>
                <span class="help">
                    <?php _e('Ticking the Enabled will show this predefined personal information question on the corresponding tab. Ticking Required will make it compulsory', 'fbsr'); ?>
                </span>
            </td>
        </tr>
        <?php endforeach; ?>
...
<?php for($i = 0; $i <= 19; $i++) : ?>
<div class="pinfo-<?php echo $i; ?>">
    <table class="form-table">
        <tbody>
            <tr>
                <th scope="row">
                    <label for="pinfo_<?php echo $i; ?>_enabled"><?php _e('Enabled?', 'fbsr'); ?></label>
                </th>
                <td>
                    <?php $this->print_checkbox('pinfo[' . $i . '][enabled]', '1', $op[$i]['enabled'] == true); ?>
                </td>
                <td>
                    <span class="help">
                        <?php _e('Tick this to show this on personal information form.', 'fbrs'); ?>
                    </span>
                </td>
            </tr>
            <tr>
                <th scope="row">
                    <label for="pinfo_<?php echo $i; ?>_question"><?php _e('Question', 'fbsr'); ?></label>
                </th>
                <td>
                    <?php $this->print_input_text('pinfo[' . $i . '][question]', $op[$i]['question'], 'large-text'); ?>
                </td>
                <td>
                    <span class="help">
                        <?php _e('Enter the question.', 'fbsr'); ?>
                    </span>
                </td>
            </tr>
            <tr>
                <th scope="row">
                    <label for="pinfo_<?php echo $i; ?>_type"><?php _e('Question Type', 'fbsr'); ?></label>
                </th>
                <td>
                    <select name="pinfo[<?php echo $i; ?>][type]" id="pinfo_<?php echo $i; ?>_type">
                        <?php $this->print_select_op($pinfo_type, $op[$i]['type'], true); ?>
                    </select>
                </td>
                <td>
                    <span class="help">
                        <?php _e('Enter the type of the answer field:'); ?>
                        <ul class="ul-square">
                            <li><strong><?php _e('Single Value from the Options', 'fbsr'); ?></strong> : <?php _e('User will need to select a value from the options list.', 'fbsr') ?></li>
                            <li><strong><?php _e('Multiple Value from the Options', 'fbsr') ?></strong> : <?php _e('User can select more than one value from the options list.', 'fbsr') ?></li>
                            <li><strong><?php _e('Free Type Text using a small Input Box', 'fbsr') ?></strong> : <?php _e('User will need to type their answer using a small input text box without any line breaks. Please see the example below', 'fbsr') ?><br /><label style="font-weight: bold">Example Question/Label:</label> <br /><input type="text" class="regular-text" value="Text Goes Here..." /></li>
                            <li><strong><?php _e('Free Type Text using a Textarea', 'fbsr') ?></strong> : <?php _e('User will need to type their answer inside a textarea, where they can have line breaks.', 'fbsr') ?><br /><label style="font-weight: bold">Example Question/Label:</label> <br /><textarea class="regular-text widefat">Text Goes Here...</textarea></li>
                        </ul>
                    </span>
                </td>
            </tr>
            <tr>
                <th scope="row">
                    <label for="pinfo_<?php echo $i; ?>_options"><?php _e('Answer Options', 'fbsr'); ?></label>
                </th>
                <td>
                    <?php $this->print_textarea('pinfo[' . $i . '][options]', $op[$i]['options'], 'large-text', 4); ?>
                </td>
                <td>
                    <span class="help">
                        <?php _e('Enter the answers of the questions. One answer at a line.', 'fbsr'); ?>
            ...
            <tr>
                <th scope="row">
                    <label for="pinfo_<?php echo $i; ?>_required"><?php _e('Compulsory?', 'fbsr'); ?></label>
                </th>
                <td>
                    <?php $this->print_checkbox('pinfo[' . $i . '][required]', 'true', ($op[$i]['required'] == true)); ?>
                </td>
                <td>
                    <span class="help">
                        <?php _e('Tick if you want to make  this question compulsory, otherwise Untick.'); ?>
                    </span>
                </td>
            </tr>
        </tbody>
...
class wp_feedback_view extends wp_feedback_base {
    ...
    public function admin_menu() {
        $this->pagehook = add_submenu_page('wp_feedback_dashboard', __('View a Feedback', 'fbsr'), __('View a Feedback', 'fbsr'), $this->capability, 'wp_feedback_view', array(&$this, 'index'));
        parent::admin_menu();
    }
    public function index() {
	...
	'<p>' . sprintf(__('<a href="%s" target="_blank">Documentation</a>', 'fbsr'), wp_feedback_loader::$documentation) . '</p>' .
	'<p>' . sprintf(__('<a href="%s" target="_blank">Support Forums</a>', 'fbsr'), wp_feedback_loader::$support_forum) . '</p>' .
        '<p>' . '<a href="http://codecanyon.net/item/wp-feedback-survey-quiz-manager-pro/3180835?ref=iPanelThemes">' . __('Go Pro', 'fbsr') . '</a></p>'
	);
    }
...
jQuery(document).ready(function($) {
    ...
    var info_count = <?php echo json_encode((object) $info_count); ?>;
...
<p>
    <span style="float: right">
        <span class="help">
            <?php _e('Both the versions should be same, the first one says the version which the script is running, the second one says the version stored in database. If they fail to match, then please just deactivate and activate the plugin. If it persists, then contact the developer.', 'fbsr'); ?>
        </span>
    </span>
    <?php printf(__('<strong>Plugin Version:</strong> <em>%s</em> (Script Version)/<em>%s</em> (Database Version)', 'fbsr'), wp_feedback_loader::$version, $wp_feedback_info['version']); ?>
</p>
<div class="p-message green">
    <a href="http://codecanyon.net/item/wp-feedback-survey-quiz-manager-pro/3180835?ref=iPanelThemes"><img style="max-width: 95%; height: auto; display: block; margin: 0 auto 10px;" src="<?php echo $this->url['images'] . '/fsqm-preview.jpg' ?>" /></a>
    <h4><?php _e('Go Pro', 'fbsr') ?>:</h4>
    <p><?php _e('If you like the free version, then you will LOVE the pro version. Here are some of the great features of the Pro version:', 'fbsr') ?></p>
    <ul class="ul-disc">
        <li><?php _e('Create unlimited forms, each with its own set of settings.', 'fbsr') ?></li>
        <li><?php _e('24 Unique Themes to choose from. Easily create your own style using our intuitive style builder (no css knowledge required, just pick colors, image and done)', 'fbsr') ?></li>
        <li><?php _e('Choose between various Google Fonts. Over 20 fonts to select from.', 'fbsr') ?></li>
        <li><?php _e('More powerful Analytics system, view survey reports in Pie Chart, Column Chart, Bar Chart or detailed Tabular Data.', 'fbsr') ?></li>
        <li><?php _e('Custom User/Admin Notification System.', 'fbsr') ?></li>
        <li><?php _e('Edit/Print/Moderate/Star the submissions.', 'fbsr') ?></li>
    </ul>
    <p><?php _e('And much more... Please check the links below', 'fbsr'); ?></p>
    <p>
        <a href="http://ipanelthemes.com/fsqm/" class="button-secondary"><?php _e('View Demo', 'fbsr'); ?></a>
        <a href="http://ipanelthemes.com/fsqm-doc/" class="button-secondary"><?php _e('Documentation', 'fbsr'); ?></a>
        <a href="http://codecanyon.net/item/wp-feedback-survey-quiz-manager-pro/3180835?ref=iPanelThemes" class="button-primary"><?php _e('Buy Now $20', 'fbsr'); ?></a>
    </p>
</div>
        <?php
    }
...
<div class="wrap" id="<?php echo $this->pagehook; ?>-widgets">
    <div class="fsqm-banner">
        <a href="http://codecanyon.net/item/wp-feedback-survey-quiz-manager-pro/3180835?ref=iPanelThemes">
            <img src="<?php echo $this->url['images'] . '/fsqm-banner.jpg'; ?>" />
        </a>
    </div>
    <div class="icon32">
...
class wp_feedback_view_cb {
    ...
    public function show($id) {
        ...
        $pinfo = maybe_unserialize($result->pinfo);
        if(null === $result) {
...
<table class="widefat">
    ...
    <tbody>
        ...
        <?php foreach($this->pinfo as $key => $spinfo) : if($spinfo['enabled'] != true) continue; ?>
        <?php if(in_array($key, array('f_name', 'l_name', 'email', 'phone'), TRUE)) continue; ?>
        <tr>
            <th scope="row"><?php echo $spinfo['question']; ?></th>
            <td>
                <?php
                switch($spinfo['type']) {
                    case 'single' :
                        $options = $this->split_options($spinfo['options']);
                        echo '<ul>';
                        foreach($options as $okey => $oname) {
                            ?>
                <li><label for="pinfo_<?php echo $key . '_' . $okey; ?>">
                    <input type="radio" value="<?php echo $okey; ?>" name="pinfo[<?php echo $key; ?>]"<?php if($okey == $pinfo[$key]) echo ' checked="checked"'; ?> id="pinfo_<?php echo $key . '_' . $okey; ?>" /> <?php echo $oname; ?>
                </label></li>
                            <?php
                        }
                        echo '</ul>';
                    ...
                    case 'multiple' :
                        ...
                        foreach($options as $okey => $oname) {
                            ?>
                <li><label for="pinfo_<?php echo $key . '_' . $okey; ?>">
                    <input type="checkbox" value="<?php echo $okey; ?>" name="pinfo[<?php echo $key; ?>][]"<?php if(in_array($okey, (array) $pinfo[$key])) echo ' checked="checked"'; ?> readonly="readonly" id="pinfo_<?php echo $key . '_' . $okey; ?>" /> <?php echo $oname; ?>
                </label></li>
                            <?php
                        }
                        echo '</ul>';
                    ...
                    case 'free-input' :
                        if($this->edit == true) {
                            echo '<input type="text" value="' . esc_attr($pinfo[$key]) . '" class="large-text" name="pinfo[' . $key . ']" />';
                        } else {
                            echo $pinfo[$key];
                        }
                        break;
                    case 'free-text' :
                        if($this->edit == true) {
                            echo '<textarea class="large-text" name="pinfo[' . $key . ']">' . esc_attr($pinfo[$key]) . '</textarea>';
                        } else {
                            echo $pinfo[$key];
                        }
                        break;
                    case 'required-checkbox' :
                        echo '<input type="checkbox" name="pinfo[' . $key . ']" value="true" checked="checked" />';
                        break;
                    default :
                        var_dump($spinfo);
                }
...
<table class="widefat">
    ...
    <tbody>
        <?php foreach($this->survey as $mcqk => $mcqv) : if(true != $mcqv['enabled']) continue; ?>
        <?php $options = $this->split_options($mcqv['options']); ?>
        <tr>
            <th scope="row"><?php echo $mcqv['question']; ?></th>
            <td>
                <ul>
                    <?php foreach($options as $okey => $oname) : ?>
                    <?php
                    $checked = false;
                    if('multiple' == $this->survey[$mcqk]['type']) {
                        if(in_array((string) $okey, (array) $survey[$mcqk], TRUE))
                            $checked = true;
                    } else {
                        if($okey == $survey[$mcqk])
                            $checked = true;
                    ...
                    ?>
                    <li><label for="mcq_<?php echo $mcqk . '_' . $okey; ?>">
                    <input value="<?php echo esc_attr($okey); ?>" type="<?php echo ($mcqv['type'] == 'single' ? 'radio' : 'checkbox'); ?>" name="mcq[<?php echo $mcqk; ?>]<?php if('multiple' == $mcqv['type']) echo '[]'; ?>" id="mcq_<?php echo $mcqk . '_' . $okey; ?>"<?php echo ($checked ? 'checked="checked"' : ''); ?> />
                    <?php echo $oname; ?>
                    </label></li>
                    <?php endforeach; ?>
...
<table class="widefat">
    <thead>
        <tr>
            <th scope="col"><?php _e('Topic/Question', 'fbsr'); ?></th>
            <th scope="col"><?php _e('Feedback/Answer', 'fbsr'); ?></th>
        </tr>
    </thead>
    <tbody>
        <?php foreach($this->feedback as $fk => $f) : if(true != $f['enabled']) continue; ?>
        <tr>
            <th scope="col"><?php echo $f['name']; ?></th>
            <td>
                <?php if($this->edit == true) : ?>
                <textarea class="large-text" name="freetype[<?php echo $fk; ?>]"><?php echo esc_html($feedback[$fk]); ?></textarea>
                <?php else : ?>
                <?php echo wpautop($feedback[$fk]); ?>
                <?php endif; ?>
            </td>
        </tr>
        <?php endforeach; ?>
